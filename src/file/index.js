  ...
  function * staticSiteGenerator() {
    try {
      ...
      if(statObj != null && statObj.isDirectory != null && typeof statObj.isDirectory === 'function') {
        const shouldRemove = yield prompt.confirm(`The directory ${path} is not empty. Would you like to remove it's contents?`)
      ...
      if(isENOENT(e)) {
        const shouldCreateDir = yield prompt.confirm(`The directory ${path} does not exist. Would you like to create it now?`)
        if(shouldCreateDir) {
      ...
      console.log(`
Done.
To view the full work log visit #
Opening in browser...
      x------THE END --------x
